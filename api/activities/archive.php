...
$up = $conn->prepare("UPDATE activities SET status = 'archived', updated_at = NOW() WHERE id = ?");
$up->bind_param('i', $activity_id);
if (!$up->execute()) {
  echo json_encode(['success' => false, 'message' => 'فشل الأرشفة: ' . $up->error]);
  exit;
}
$up->close();

echo json_encode(['success' => true, 'message' => 'تمت أرشفة الإعلان بنجاح']);
